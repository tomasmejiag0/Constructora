    ...
    export default function ManagerTaskPage() {
      ...
      const { tasks, addTask, getProjectById, projectAssignments } = useAuth();
      ...
      const mockWorkers = [
        { id: 'worker-1', name: 'Worker User' },
        { id: 'worker-2', name: 'Charlie Worker' },
        { id: 'worker-3', name: 'Eva Electrician' },
        { id: 'worker-4', name: 'Frank Mason' },
      ];

      const workersOnThisProject = projectAssignments
        .filter(pa => pa.projectId === projectId)
        .map(pa => mockWorkers.find(w => w.id === pa.userId))
        .filter(Boolean);
